...
class SwitchingCriteria(PydanticBaseModel):
    """Criteria for when to switch models"""
    
    type: Literal["step_count", "token_count", "cost", "custom", "random_step_range"] = "step_count"
    """Type of switching criteria to use"""
    
    threshold: int | float = 10
    """Threshold value for switching (e.g., switch after 10 steps)"""
    
    custom_function: str | None = None
    """Python code for custom switching function (if type is 'custom').
    Should be a function that takes (model_idx, stats, step_count) and returns bool.
    """
    
    # For random_step_range type
    start_step: int | None = None
    """Start of the step range for random switching"""
    
    end_step: int | None = None
    """End of the step range for random switching"""
...
class ModelAgentConfig(PydanticBaseModel):
    ...
    templates: dict[str, Any] | None = Field(
        default=None,
        description="Template configuration overrides for this model"
    )
    
    tools: dict[str, Any] | None = Field(
        default=None,
        description="Tool configuration overrides for this model"
    )
    
    history_processors: list[dict[str, Any]] | None = Field(
        default=None,
        description="History processor configuration overrides for this model"
    )
    
    model_config = ConfigDict(extra="forbid")


class SwitchingModelConfig(PydanticBaseModel):
    """Configuration for a model that switches between multiple models"""
    
    name: Literal["switching"] = "switching"
    
    models: list[GenericAPIModelConfig] = Field(
        description="List of model configurations to switch between (legacy format)"
    )
    
    model_configs: list[ModelAgentConfig] | None = Field(
        default=None,
        description="List of model and agent configurations to switch between (new format)"
    )
    
    switching_criteria: list[SwitchingCriteria] = Field(
        default_factory=lambda: [SwitchingCriteria(type="step_count", threshold=10)],
        description="List of criteria for switching models. Models switch in order when criteria are met."
    )
    
    per_instance_cost_limit: float = Field(
        default=3.0,
        description="Cost limit for every instance (cumulative across all models).",
    )
    
    total_cost_limit: float = Field(
        default=0.0, 
        description="Total cost limit (cumulative across all models)."
    )
    
    per_instance_call_limit: int = Field(
        default=0, 
        description="Per instance call limit (cumulative across all models)."
    )
    
    reset_history_on_switch: bool = Field(
        default=False,
        description="Whether to reset conversation history when switching models"
    )
    
    @model_validator(mode='after')
    def validate_model_configs(self) -> Self:
        """Ensure either models or model_configs is provided"""
        if self.model_configs is None:
            # Convert legacy format to new format
            self.model_configs = [
                ModelAgentConfig(model=model) for model in self.models
            ]
        return self
...
class LiteLLMModel(AbstractModel):
    ...
    def _single_query(
        ...
        try:
            custom_llm_provider = None  # Initialize to avoid UnboundLocalError
...
def get_model(args: ModelConfig | dict, tools: ToolConfig) -> AbstractModel:
    """Returns correct model object given arguments and commands"""
    
    # Handle dict input (for dynamic configuration loading)
    if isinstance(args, dict):
        # Check if it's a switching model
        if args.get("name") == "switching":
            from sweagent.agent.switching_model import SwitchingModel, SwitchingModelConfig
            args = SwitchingModelConfig(**args)
            return SwitchingModel(args, tools)
        # Convert dict to appropriate model config
        # name = args.get("name", "")
        # if name == "human":
        #     args = HumanModelConfig(**args)
        # elif name == "human_thought":
        #     args = HumanThoughtModelConfig(**args)
        # elif name == "replay":
        #     args = ReplayModelConfig(**args)
        # elif name == "instant_empty_submit":
        #     args = InstantEmptySubmitModelConfig(**args)
        # else:
        #     args = GenericAPIModelConfig(**args)
    
    # Handle switching model with dynamic import to avoid circular imports
    if hasattr(args, 'name') and args.name == "switching":
        from sweagent.agent.switching_model import SwitchingModel, SwitchingModelConfig
        if not isinstance(args, SwitchingModelConfig):
            # Try to convert if it's a generic config with switching name
            if isinstance(args, GenericAPIModelConfig):
                # This might happen if the config parser doesn't recognize SwitchingModelConfig
                # We'll need to reconstruct it from the args
                try:
                    args_dict = args.model_dump() if hasattr(args, 'model_dump') else vars(args)
                    args = SwitchingModelConfig(**args_dict)
                except Exception as e:
                    raise ValueError(f"Failed to convert {type(args)} to SwitchingModelConfig: {e}") from e
        return SwitchingModel(args, tools)
